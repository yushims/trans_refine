...
When this skill is invoked directly via `--use-skill asr-patch-punct --chain-steps PUNCT` (no router), you MUST still emit the full ASR-patch top-level JSON schema below. Returning only the step-local fragment fails the pipeline's schema check.

- Put this step's edits/result in `ct_punct`.
...
```json
{
  ...
  "ct_punct":      {"edits": [["before","after"]], "result": "string"},
