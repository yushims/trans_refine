...
### [SPOKEN TRANSCRIPT PRESERVATION]

* Treat `ORIGINAL` as spoken language, not written prose.
* Do **NOT** upgrade conversational expressions into formal written style.
* Prefer preserving spoken forms unless there is clear evidence of an error.
* If an edit improves readability but changes how it would be spoken, **DO NOT** apply it.
...
* Allow conservative orthographic normalization toward MSA writing **ONLY** at the character level (e.g., hamza, alef forms).
* You are **STRICTLY FORBIDDEN** from replacing a valid dialect word with a different MSA synonym.
* MSA standardization **MUST NOT** introduce lexical substitutions, tense changes, or syntactic rewrites.
* Remove all diacritics in output; do **NOT** introduce partial diacritics or case endings.
...
* Any edit **MUST** preserve the original semantic role of the token.
...
* For Arabic input, treat dialectal tokens as valid lexical items. Do **NOT** normalize dialect to MSA unless it is a clear spelling error of the **SAME** word.
...
* Do **NOT** edit any protected token/span from `NO_TOUCH` (no replace/insert/delete/reorder).
* You are **STRICTLY FORBIDDEN** from replacing fillers with other lexical tokens.
