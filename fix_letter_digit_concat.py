"""Fix missing spaces at letter-digit concatenation points.

Catches cases where numeral conversion glued digits to adjacent words,
e.g. "augusztus29" → "augusztus 29", "számom06" → "számom 06".

Three-phase workflow:
  extract  Scan for letter+digit concatenations (no space), deduplicate, write patterns JSON.
  judge    Send unique patterns to LLM for YES/NO space verdicts.
  apply    Apply verdicts to fix the output file.

Usage:
  python fix_letter_digit_concat.py extract --input outputs/file.tsv --output patterns.json
  python fix_letter_digit_concat.py judge   --patterns patterns.json --output verdicts.json
  python fix_letter_digit_concat.py apply   --input outputs/file.tsv --verdicts verdicts.json --output fixed.tsv
"""
...
# ---------------------------------------------------------------------------
# Scanning: find letter+digit concatenation points (no space)
# ---------------------------------------------------------------------------
...
def scan_letter_digit_concat_candidates(text: str) -> list[dict]:
    """Find positions where a letter is directly followed by a digit (no space).

    Returns dicts with keys: pos, key, left, right.
    *pos* is the index of the first digit character (insertion point for space).
    """
    ...
    while i < length:
        ...
        if not (prev_ch.isalpha() and curr_ch.isdigit()):
            i += 1
            continue

        # Extract the letter-word before and digit-run after.
        word_before = _extract_token(text, i, -1)
        digit_after = _extract_token(text, i, +1)

        if not word_before or not digit_after:
            i += 1
            continue

        # Skip short letter parts — likely intentional compounds (mp3, v2, H264).
        # Only flag when letter part is 3+ chars or digit part is 2+ digits.
        if len(word_before) < 3 and len(digit_after) < 2:
            i += len(digit_after)
            continue

        key = f"{word_before.casefold()}|{digit_after}"

        left_start = max(0, i - 30)
        right_end = min(length, i + 30)
        left_ctx = text[left_start:i]
        right_ctx = text[i:right_end]
...
def run_extract(args: argparse.Namespace) -> None:
    ...
    with input_path.open("r", encoding="utf-8") as f:
        for line_num, raw_line in enumerate(f, 1):
            ...
            lines_scanned += 1
            candidates = scan_letter_digit_concat_candidates(text)
...
def _build_judge_prompt(batch_patterns: list[dict], minimal_context: bool = False) -> str:
    ...
    return (
        ...
        "a letter and digit are concatenated without a space (marked with \u25b8). "
        "A space may be missing.\n\n"
        "For each snippet, determine if a space should be INSERTED at the \u25b8 position.\n\n"
        "Rules:\n"
        "- INSERT: If the digit was converted from a numeral word and got glued to "
        "the adjacent word (e.g. 'augusztus29' should be 'augusztus 29', "
        "'sz\u00e1mom06' should be 'sz\u00e1mom 06')\n"
        "- INSERT: If the letter part is a regular word (3+ chars) and the digit is "
        "clearly a separate number\n"
        "- KEEP: If letter+digit form a known compound, brand, version, unit, or "
        "identifier (e.g. 'mp3', 'v2', 'H264', 'iPhone6', 'USB3', 'WiFi6')\n"
        "- KEEP: If the concatenation is intentional in that language\n\n"
        "Respond ONLY with a JSON object mapping the item number (as string) to "
        '"INSERT" or "KEEP".\n'
        'Example: {"1": "INSERT", "2": "KEEP"}\n\n'
        "Items:\n" + "\n".join(items)
    )
...
async def run_judge(args: argparse.Namespace) -> None:
        ...
        async with semaphore:
            ...
            try:
                ...
                for idx, p in enumerate(batch_patterns, 1):
                    ...
                    verdicts[p["key"]] = verdict_str == "INSERT"

                insert_in_batch = sum(
                    1
                    for idx in range(1, len(batch_patterns) + 1)
                    if str(result.get(str(idx), "")).strip().upper() == "INSERT"
                )
                print(
                    f"  Batch {batch_idx + 1}: "
                    f"{len(batch_patterns)} patterns, "
                    f"{insert_in_batch} INSERT / {len(batch_patterns) - insert_in_batch} KEEP"
                )
    ...
    for retry_round in range(1, 4):
        unresolved = [p for p in remaining if verdicts.get(p["key"]) is None]
        if not unresolved:
            break
        use_minimal = any(p["key"] in filtered_keys for p in unresolved)
        print(f"\n  Auto-retry {retry_round}: {len(unresolved)} unresolved patterns"
    ...
    insert_count = sum(1 for v in verdicts.values() if v is True)
    ...
    output_data = {
        ...
        "space_insert": insert_count,
        "space_keep": keep_count,
        "unresolved": unresolved_count,
        "verdicts": {
            k: ("INSERT" if v is True else "KEEP" if v is False else "UNRESOLVED")
    ...
    print(
        f"\nVerdicts: {insert_count} INSERT, {keep_count} KEEP, {unresolved_count} unresolved"
    )
    if failed_batches:
...
def _load_verdicts(verdicts_path: Path) -> dict[str, bool]:
    ...
    return {
        k: (v == "INSERT" if isinstance(v, str) else bool(v))
...
def apply_verdicts_to_text(
    ...
    """Insert spaces where verdicts say INSERT. Returns (fixed_text, num_fixes)."""
    candidates = scan_letter_digit_concat_candidates(text)
    if not candidates:
        return text, 0
    ...
    for c in candidates:
        ...
        if verdict is True:
            insert_positions.add(c["pos"])

    if not insert_positions:
        return text, 0

    # Build result by inserting space at insert positions.
...
def run_apply(args: argparse.Namespace) -> None:
    ...
    insert_count = sum(1 for v in verdicts.values() if v)
    print(f"Loaded {len(verdicts)} verdicts ({insert_count} INSERT)")
    ...
    print(f"Fixed {lines_fixed:,} lines ({total_fixes:,} spaces inserted).")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Fix missing spaces at letter-digit concatenation points.",
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    # -- extract --
    extract_parser = subparsers.add_parser(
        "extract", help="Scan output for letter+digit concatenation patterns"
    )
